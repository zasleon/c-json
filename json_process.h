...
#pragma comment(lib,"F:\\JY\\jsoncpp-src-0.5.0\\jsonPP\\lib\\debug\\json.lib")//链接需要的库文件
using namespace std;
...
可将以下代码放入main函数中测试
	...
	Json::FastF:\\JY\\jsoncpp-src-0.5.0\\jsonPP\\lib\\debugWriter writer;//裸建一个json写入string的接口
...
class JSON_package
{
	private:
		...
		bool whether_normal;//是否正常，不正常不处理
	public:
		...
		void add_item_array(string name,string value);//将某一单值以数组数据方式存入属性里
		...
		void add_item_array(string array_name,string name,int value);//将某一单值以数组数据方式存入array_name属性里
...
void process_JSON_string(char* value)//json返回的value字符串内容往往开头和结尾是双引号且后一个双引号后以\n结尾
{
	value[strlen(value)-2]='\0';//最后一个是\n,倒数第二个是\"，所以把倒二设为\0

	for(int j=0;value[j]!='\0';j++)//第一个是\"，所以后面的全部前移一个把首个\"覆盖
		value[j]=value[j+1];
	return;
}
